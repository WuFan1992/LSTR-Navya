...
class TUSIMPLE(DETECTION):
    def __init__(self, db_config, split):
        ...
        print("anno file = ", self.anno_files)
    ...
    def _transform_annotation(self, anno, img_wh=None):
        ...
        lanes[:, 0] = 0
        old_lanes = sorted(old_lanes, key=lambda x: x[0][0][0]) # sorted according to the x 
        for lane_pos, (lane, category) in enumerate(old_lanes):
            ...
            lanes[lane_pos, 3:3 + len(xs)] = xs       # from index 3 to 59 only the x  
            lanes[lane_pos, (3 + self.max_points):(3 + self.max_points + len(ys))] = ys  # from index 60 to 115 only y
