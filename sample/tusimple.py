...
def kp_detection(db, k_ind):
    ...
    images   = np.zeros((batch_size, 3, input_size[0], input_size[1]), dtype=np.float32) # b, 3, 360(H),640(W)
    masks    = np.zeros((batch_size, 1, input_size[0], input_size[1]), dtype=np.float32)  # b, 1, 360, 640
    gt_lanes = []
    ...
    for b_ind in range(batch_size):
        ...
        item  = db.detections(db_ind) # get the image according to the id which is shuffled
